...
public class TimeHelper {
    ...
    public static String date2TimeStamp(String date_str) {
        return String.valueOf(TimeHelper.date2TimeStamp(date_str, "yyyy-MM-dd"));
    }
    ...
    public static long getAddMinutesLeaveMillSec(LocalDateTime now, long minutes) {
        ...
        long timeSpan = TimeHelper.date2TimeStampMills(d2, strFormat) - TimeHelper.date2TimeStampMills(d1, strFormat);
        return timeSpan;
    }
     ...
     * @return
     */
    public static List<String> getWeekDays(Long timeStamp13){
        Date date = new Date();
    ...
    public static Map getMonthNoAndWeekNo(Long timeStamp13) {

        LocalDateTime sourceTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(timeStamp13), ZoneId.systemDefault());

        Map map = new HashMap<>();

        String monthNoAndWeekNo;//获取当月的第一天
        ...
        if (!firstMondayOfMonth.isBefore(sourceTime)) {//如果当月的第一个周一大于参数时间,则要计算到上个月份去//获取上一个月的第一个周一
            ...
            int diffDays = dayOfMonthSourceTime - dayOfMonthFirstMonday;//第几周weekNo
    ...
    public static LocalDateTime getLocalDateTime(Long timeStamp13){
        return LocalDateTime.ofInstant(Instant.ofEpochMilli(timeStamp13), ZoneId.systemDefault());
    }
